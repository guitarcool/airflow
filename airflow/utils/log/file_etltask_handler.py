...
class FileETLTaskHandler(logging.Handler):
    """
    FileETLTaskHandler is a python log handler that handles and reads
    etl re-run task logs. It creates and delegates log handling
    to `logging.FileHandler` after receiving etl task context.
    It reads logs from task's host machine.
    """
    ...
    def __init__(self, base_log_folder, filename_template):
        ...
        """
        super(FileETLTaskHandler, self).__init__()
        self.handler = None
        self.local_base = base_log_folder
        self.filename_template, self.filename_jinja_template = \
            parse_template_string(filename_template)

    def set_context(self, etl_task):
        """
        Provide etl_task context to airflow task handler.
        :param etl_task: etl_task object
        """
        local_loc = self._init_file(etl_task)
