var AdminFilters = function(element, filtersElement, filterGroups, activeFilters) {
    ...
    function addFilter(name, subfilters, selectedIndex, filterValue, label) {
    ...
    $('a.filter', filtersElement).click(function() {
        var name = ($(this).data('value').trim !== undefined ? $(this).data('value').trim() : $(this).data('value').replace(/^\s+|\s+$/g,''));
        var label = ($(this).text().trim !== undefined ? $(this).text().trim() : $(this).text().replace(/^\s+|\s+$/g,''));
        addFilter(name, filterGroups[name], false, null, label);
    ...
    $.each(activeFilters, function( activeIndex, activeFilter ) {
        var idx = activeFilter[0],
            ...
            label = name;
        var $activeField = addFilter(name, filterGroups[name], idx, filterValue, label);
    });
